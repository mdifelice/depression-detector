...
class TrainingDataset:
	def __init__( self, url, target_column, dataset_file = None ):
		parsed_url = urlparse( url )

		if parsed_url.hostname == "www.kaggle.com":
			dataset_id = re.sub( r"^\/datasets\/(.+)\/?$", r"\1", parsed_url.path )
			basename = dataset_id.split( "/" )[-1]
			dataset_folder = get_data_folder( "datasets" ) + basename
			zip = dataset_folder + "/" + basename + ".zip"

			kaggle.api.dataset_download_files( dataset_id, dataset_folder )

			with ZipFile( zip , "r") as zip_object:
				zip_object.extractall( dataset_folder )

			os.remove( zip )

			if dataset_file:
				dataset_file = dataset_folder + "/" + dataset_file
		...
		if callback and dataset_file:
			data = callback( dataset_file )
		else:
