...
```bash
cp .env.example .env
# edit .env with your Google OAuth credentials and production settings
docker compose up --build
```

The app will be available at `http://localhost`. To use a different port, update the `ports` mapping in `docker-compose.yml`:
...
Data and trained models are persisted in Docker volumes (`app-data`, `app-models`).
...
| `GOOGLE_REDIRECT_URI` | OAuth callback URL | `http://localhost/auth/callback` |
| `SECRET_KEY` | Session encryption key | `change-me-in-production` |
| `FRONTEND_URL` | Frontend base URL for redirects | `http://localhost` |
| `ALLOWED_ORIGINS` | Comma-separated CORS origins | `http://localhost:5173` |

### Whitelist Management

After deployment, manage whitelisted emails from inside the backend container:

```bash
docker compose exec backend python cli.py add user@example.com
docker compose exec backend python cli.py list
docker compose exec backend python cli.py remove user@example.com
```
